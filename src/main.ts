...
function addUpdateEntry(updates: YAML.YAMLSeq) {
    return (dir: string) => {
        ...
        map.set(keyEcosystem, ecosystemGoMod);
        map.set(keySchedule, schedule);
        updates.items.push(map);

        core.info(`Added Dependabot entry for Go module in ${dir}.`);
    };
}
...
async function run(): Promise<null | Error> {
    const paths = await globby(["**/go.mod", "!**/vendor/**/go.mod"], {
        gitignore: true,
    });

    if (paths.length == 0) {
        core.info("No Go modules found.");
        return null;
    }
    ...
    if (fs.existsSync(cfgFile)) {
        ...
        for (const i in updates.items) {
            ...
            if (update.get(keyEcosystem) !== ecosystemGoMod) {
                continue;
            }
